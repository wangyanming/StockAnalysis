...
def render_report(data: dict) -> str:
    ...
    if pk:
        ...
        parts.append(f"📝 评分说明: 筹码结构(25分)+资金接力(25分)+板块环境(12分)+趋势位置(14分)+大盘安全(10分)+位置评分(+15分)")   # 2026-08-19：板块 20→12
        ...
        for g_key in ['B', 'C', 'D']:
            ...
            for i, s in enumerate(members, 1):
                ...
                dim_str = ' | '.join(f"{k}{dims.get(k, 0)}/{max_s}"
                                      for k, max_s in [('筹码', 25), ('接力', 25), ('板块', 12), ('趋势', 14), ('大盘', 10)]   # 2026-08-19：板块满分 20→12
                                      if k in dims)
                if '位置' in dims:
