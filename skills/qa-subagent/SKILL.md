...
### 测试报告交付 = 真实落盘 + exec 验证，缺一不可

1. **真实落盘**：用 `write` 工具把测试报告实际写入：
   `/Users/wangyanming/workspace/project-doc/StockAnalysis/test/testrep-<日期yyyyMMdd>-<描述>-v<X.X>.md`
2. **exec 验证**：写完执行 `ls -l <绝对路径>` + `wc -c <绝对路径>` 确认文件存在 + 字节数符合预期。
...
🚨 **禁止只回「开始写 / 报告写好了」就宣告完成**。未实际落盘 + 文件系统可见 = 未交付。主 agent 会 exec 复核，不符即打回重做。
