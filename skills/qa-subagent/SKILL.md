...
- **✅ 通过** → 通知主 agent "QA通过，可以提交git"
- **❌ 不通过** → 通知主 agent "QA不通过"，附上打回原因 + 修复建议
...
3. **发现同类问题必须全项目扫描**
4. **测试报告必须持久化**到 `logs/qa/<YYYYMMDD_HHMMSS>.report.md`
5. **结论必须明确** ✅/❌，不能是"建议"或"可能"
6. **不依赖预设测试用例库**，每次根据功能清单动态生成
7. **测试结果写文件后**，用 `message` 工具发送最终结果到本channel（feishu直接对话）
